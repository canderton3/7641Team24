...
Logistic regression is a soft classification method that relys on feature based selection to determine a probability between two "possibilities." The basic concept to logistic regression is to intake various features and to output probabilities between two scenarios on an ROC curve to assign labels. Even though it is soft classification method, the team still used a hard classification to whichever label held a majority probability. Using a logistic definition between the two options of false positive and candidate planet helps the team determine if a potential exoplanet requires further evaluation.
